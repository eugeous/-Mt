...
void XoanZiczacTang(float[][500], int, int);
...
int main()
{
	...
	for (int i = 1; i <= 13; i++)
	{
		...
		Xuat(b, k, l);
		cout << '\n' << '\n';
		XoanZiczacTang(b, k, l);
		cout << "\nMang sau khi sap xep tang theo chieu xoan ziczac la: ";
		Xuat(b, k, l);
...
void XoanZiczacTang(float a[][500], int m, int n)
{
	float b[500];
	int k;
	int i, j;
